...
export default function Input() {
  ...
  useEffect(() => {
    const balance = calculateTotalBalance().toLocaleString("id-ID");
    const totalB =calculateTotalBalance()
  ...
  const handleDoneClick = () => {
    const formattedData = {
      ...
      detail: expenseDetails.filter(
        (detail) =>
          detail.pembelian !== "" && detail.harga !== "" && detail.qty !== ""
      ),
    };
  ...
  return (
    <div className="container">
      ...
      <div className="grid grid-cols-2 gap-5 my-10">
          ...
          {expenseDetails.map((detail, index) => (
            <div key={index} className="grid grid-cols-3 gap-2 mt-2 mr-5">
              <div>
                <label className="form-control w-full max-w-xs">
                  <div className="label">
                    <span className="label-text">Pembelian</span>
                  </div>
                  <input
                    type="text"
        ...
        <div>
          <h1 className="font-bold">Shopping Expense</h1>
          <div className="mt-3">
            <div className="stats bg-yellow-600 text-primary-content">
              <div className="stat">
